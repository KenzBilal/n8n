...
export default function Inbox() {
  const [repliedCompanies, setRepliedCompanies] = useState<any[]>([]);
  const [selectedCompany, setSelectedCompany] = useState<any | null>(null);
  const [thread, setThread] = useState<any[]>([]);
  const [drafts, setDrafts] = useState<any[]>([]);

  useEffect(() => {
    fetchRepliedCompanies();
  }, []);
  ...
  async function fetchRepliedCompanies() {
    const { data } = await supabase
      .from('companies')
      .select('*')
      .eq('status', 'REPLIED')
      .order('created_at', { ascending: false });
    if (data) setRepliedCompanies(data);
  }
    ...
    <div className="flex h-screen bg-[#0F0F0F] text-zinc-300 font-inter">
      ...
      <div className="w-1/3 border-r border-zinc-800/50 flex flex-col">
        <div className="p-6 border-b border-zinc-800/50">
          <h2 className="text-xl font-medium tracking-tight text-white">Active Replies</h2>
        </div>
        <div className="overflow-y-auto flex-1">
          {repliedCompanies.length === 0 ? (
            <div className="p-6 text-zinc-500 text-sm">No active replies yet.</div>
          ) : (
            repliedCompanies.map(c => (
              <div 
                key={c.id}
